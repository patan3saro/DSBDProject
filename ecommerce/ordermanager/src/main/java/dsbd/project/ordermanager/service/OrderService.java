...
public class OrderService {
    ...
    public String add(OrderRequest orderRequest, int userId, HttpServletRequest request) throws ServerException, ClientException { //Ci serve per ottenere X-User-ID
        try {
            ...
            throw new ClientException(request, loggingTopic, httpErrorsTopicKey, e.getStatus());
        }
        catch(Exception e){
            throw new ServerException(request,loggingTopic, httpErrorsTopicKey);
        }
    }

    public Page<FinalOrder> getAllOrders(int userId,int per_page, int page, HttpServletRequest request) throws ClientException, ServerException {
        try {
            String USER_MANAGER_URL = eurekaClient.getNextServerFromEureka("usermanager", false).getHomePageUrl();
    ...
    public Optional<FinalOrder> getId(Integer id, int userId, HttpServletRequest request) throws ClientException, ServerException {
        try {
            String USER_MANAGER_URL = eurekaClient.getNextServerFromEureka("usermanager", false).getHomePageUrl();
            if (userId != 0) {
                User user = new RestTemplate().getForObject(USER_MANAGER_URL + "/user/id/{userId}", User.class, userId);
